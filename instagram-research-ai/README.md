...
## 本番デプロイ（Vercel + Supabase）— ターミナル操作は不要です

1. **Supabase**（https://supabase.com）でプロジェクトを作成 → 「Connect」ボタンから接続文字列を2つコピー
   - `DATABASE_URL` ← Transaction pooler（ポート6543）のURL
   - `DIRECT_URL` ← Direct connection（ポート5432）のURL
   - どちらも `[YOUR-PASSWORD]` の部分をプロジェクト作成時に決めたDBパスワードへ置き換える
2. **Vercel**（https://vercel.com）でこのGitHubリポジトリをImport
   - **Root Directory を `instagram-research-ai` に設定**（Editから変更）
   - Environment Variables に以下を貼り付け:
     `DATABASE_URL` / `DIRECT_URL` / `AUTH_SECRET`（ランダム32文字以上） / `CRON_SECRET`（同） / `ANTHROPIC_API_KEY` / `AI_PROVIDER=anthropic` / `DATA_SOURCE=mock` / `AUTH_TRUST_HOST=true`
   - Deployを押す。**ビルド時にDBスキーマは自動適用されます**（`vercel-build` が `prisma migrate deploy` を実行）
3. 発行されたURLへアクセスすると**初回セットアップ画面**が開くので、管理者アカウントを画面上で作成（一度だけ有効。以後は通常ログイン）
4. スマホでは発行URLを開いて「ホーム画面に追加」でアプリのように利用可能。シェア連携は設定画面の手順に従う
5. 定期ジョブ: `vercel.json` に日次のスコア再計算+レポート生成を定義済み（VercelはCRON_SECRETを自動でAuthorizationヘッダーに付与）。Hobbyプランは1日1回まで。15〜30分間隔で回す場合は cron-job.org 等から `POST /api/internal/jobs/<job>`（`Authorization: Bearer <CRON_SECRET>`）を呼ぶ

CLIが使える場合は `scripts/createUser.ts` で追加ユーザー作成、`npm run seed` でモックデータ投入も可能です。
